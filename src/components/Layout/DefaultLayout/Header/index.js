...
function Header() {
  ...
  const nn = { 1: "TourTrongNuoc", 2: "TourNuocNgoai" };

  useEffect(() => {
    if (
      localStorage.getItem("account") ||
      localStorage.getItem("account-google")
    ) {
      setvalue(true);
    }
  }, [set]);
    ...
    <div className={cx("wrapper")}>
      ...
      <div className={cx("menu-bar")}>
        ...
        <ul>
          ...
          <li>
            <Link to="/chung-toi">Chúng Tôi</Link>
          </li>
          <li>
            <Link>Hợp Tác</Link>
          </li>
      ...
      <label for="checkboxbar" className={cx("menu-bar-overlay")}></label>
      ...
      <div className={cx("warpper-link-icon")}>
        {set ? (
          <div className={cx("warpper-link-icon-login")}>
            <div>
              <h5>{localStorage.getItem("account")}</h5>
            </div>
            <div>
              {localStorage.getItem("image") ? (
                <label for="checkavatar">
                  <img
                    className={cx("warpper-link-icon-avatar")}
              ...
              <input
                className={cx("warpper-link-icon-check")}
                type="checkbox"
                id="checkavatar"
              ></input>
