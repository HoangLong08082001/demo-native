        ...
        .then((response) => {
          if (response.data === "success") {
            setTimeout(() => {
              setham(false);
              navigate(
                `/user/billdetail/${params.get("matour")}?email='on'&MaPhieu=${
                  response.MaPhieu
                }`
              );
            }, 2000);
      ...
      <div className={cx("wrapper")}>
        ...
        <div>
          ...
          <div className={cx("wrapper-box2")}>
            <Button onClick={handlepre} login>
              Trở Lại
            </Button>
            {payment === 0 ? (
              <Button onClick={handlemoney} login>
                Hoàn Tất
              </Button>
            ) : (
              <Button onClick={handlemoneyonline} login>
                Thanh Toán{" "}
              </Button>
            )}
          </div>
          {statuspayment ? (
            <Alert
              dataprops={"Thanh Toán Thành Công"}
              good={0}
              icon={0}
              url={`/user/billdetail/${information.MaTour}?email='on'&MaPhieu=${MaPhieu}`}
            />
          ) : (
            ""
          )}
