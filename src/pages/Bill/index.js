...
export default function Bill() {
  ...
  useEffect(() => {
    axios.post(`tour/alltour/gettourbill`,{MaTour:id,NgayDi:date}).then((response) => {
  ...
  const handlebook=()=>{
    ...
    if(data.name ==="" || data.email ==="" || data.sdt === "" ||data.countprice === 0 ||statuspay===null || statusdk === false)
    {
        seterror(true);
    }
    // if(statuspay===0)
 ...
 const handlename=(name,email,sdt,diachi,countprice,Person,Personmin,Personbe)=>{
    setttdata({
      name:name,
      email:email,
      sdt:sdt,
  ...
  return (
    <div className={cx("wrapper")}   >
      {value.map((value) => {
        ...
        return (
          <div className={cx("wrapper-tour")} style={{ backgroundImage: `url(data:image/jpeg;base64,${base64String5})` }}  >
        <div className={cx("background-tour")}  style={{ backgroundImage: `url(data:image/jpeg;base64,${base64String5})` }}>

        </div>
        <div className={cx("background-tour-detail-tittle-css")}  >
        <div className={cx("background-tour-detail-tittle")}>
            <div><h2>{value.TenTour}</h2>
            </div>
        </div>
        ...
        <div className={cx("background-tour-detail")}>
          <div className={cx("background-tour-detail-price")} >Giá Từ {" "+price}</div>
        </div>
        </div>
      ...
      <Booking Price={valueobject.GiaTour} Giam={giam} MaTour={valueobject.MaTour} callBackname={handlename} />
      ...
      <div className={cx("btn")}>
        {nextpage===1 ?(  <Button onClick={handlebook} underline Booking to={`/Confirm?name=${data.name}&email=${data.email}&sdt=${data.sdt}&diachi=${data.diachi}&payment=${statuspay}&summoney=${data.countprice}
        ...
        &tentour=${valueobject.TenTour}`}>Đặt  Tour</Button>):(  <Button onClick={handlebook} underline Booking >Đặt  Tour</Button>)}
        <Button Cancel>Hủy</Button>
      </div>
     {adderror ? ( <Alert callBackParent={handleclose} datatt={arraytdata}  dataprops={status} icon={1}/>):("")}
    </div>
  );
}
